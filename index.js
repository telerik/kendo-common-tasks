...
const startDocsServer = (gulp, libraryName, done) => {
    ...
    const app = express();
    const vuePlatform = /vue/.test(libraryName) ? 'vue' : null;
    const reactPlatform = /react/.test(libraryName) ? 'react' : null;
    const platform = argv.platform || reactPlatform || vuePlatform || 'angular';
...
exports.startDocsServer = startDocsServer;
...
exports.addTasks = (gulp, libraryName, srcGlob, webpackConfig, dtsGlob, options = {}) => { //eslint-disable-line max-params
    ...
    gulp.task('docs', [ 'lint-slugs', 'build-cdn', 'build-npm-package', 'build-systemjs-bundle' ], (done) => startDocsServer(gulp, libraryName, done));
};
